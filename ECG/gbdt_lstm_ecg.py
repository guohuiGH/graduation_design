...
class LstmParam:
    def __init__(self,dataset):
        ...
        # Network Parameters
        self.n_input = 40 # feature number
        self.n_steps = 1
        ...
        self.f_input = 140 # input feature number
        self.thread_core = 2 # number of thread_core
        self.step = -100000
...
def train(param, model, forest_net, dataset):
    ...
    with tf.Session() as sess:
        ...
        while step  < param.training_iters:
            ...
            m2_time = int(time.time())

            redisual = param.step*np.transpose(grad_val,[1,0,2])
            e_time = int(time.time())
            #print (m1_time-s_time, m2_time-m1_time, e_time-m2_time)
        ...
        #v.calculateF1(dataset.y_input, np.array(classifier_value[0])+1)
